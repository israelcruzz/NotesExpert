...
export const NoteCard = ({ note, removeNote }: NoteCardProps) => {
  const noteDelete = () => {
    removeNote(note.id);
  };
  ...
  return (
    <Dialog.Root>
      <Dialog.Trigger className="rounded-md bg-slate-800 outline-none p-5 gap-3 overflow-hidden relative hover:ring-2 hover:ring-slate-600 focus-visible:ring-2 focus-visible:ring-lime-400 text-left flex flex-col">
      ...
      <Dialog.Portal>
        <Dialog.Overlay className="inset-0 fixed bg-black/60">
          <Dialog.Content className="fixed inset-0 md:left-1/2 md:top-1/2 md:-translate-x-1/2 md:-translate-y-1/2 md:max-w-[640px] md:w-full md:h-[60vh] bg-slate-700 md:rounded-md flex flex-col overflow-hidden">
            ...
            <button
              type="button"
              className="w-full bg-slate-800 text-center text-sm text-slate-300 outline-none font-medium group p-4"
              onClick={noteDelete}
            >
              Deseja{" "}
              <span
